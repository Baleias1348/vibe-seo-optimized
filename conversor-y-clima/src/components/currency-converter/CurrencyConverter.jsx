...
import { FiDollarSign, FiRefreshCw, FiArrowRight, FiInfo, FiAlertCircle } from 'react-icons/fi';
...
const CurrencyConverter = () => {
  ...
  const fetchRates = async () => {
    ...
    try {
      console.log('Iniciando solicitud de tasas de cambio...');
      
      // Usar una API alternativa más confiable
      ...
      if (data.result !== 'success' || !data.rates) {
        console.warn('La API no devolvió tasas válidas, usando valores por defecto');
        setRates(DEFAULT_RATES);
        setLastUpdated(new Date().toLocaleTimeString());
        throw new Error('No se pudieron obtener las tasas actualizadas');
      }
      
      // Usar las tasas de la API o los valores por defecto
      const newRates = {};
      let hasMissingRates = false;
      
      CURRENCIES.forEach(currency => {
        if (data.rates && data.rates[currency.code] && typeof data.rates[currency.code] === 'number') {
          newRates[currency.code] = data.rates[currency.code];
        } else {
          console.warn(`Tasa no encontrada para ${currency.code}, usando valor por defecto`);
      ...
      if (hasMissingRates) {
        toast.warn('Algunas tasas no están disponibles y se usaron valores por defecto', {
          position: "top-right",
          autoClose: 5000,
      ...
      if (!navigator.onLine) {
        toast.error('Sin conexión a internet. Usando tasas de cambio locales.', {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: true,
        });
      } else {
        toast.error('Error al cargar tasas actualizadas. Usando tasas locales.', {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: true,
        });
  ...
  const convertCurrency = () => {
    ...
    // Asegurarse de que las tasas estén disponibles
    if (!rates[fromCurrency] || !rates[toCurrency]) {
      setConvertedAmount('Cargando...');
      return;
    }
