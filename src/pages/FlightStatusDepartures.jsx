...
export default function FlightStatusDepartures() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3011/api/fa/departures-route/${origin}/${destination}/${date}`
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-200 py-10 px-2">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-lg p-8 border border-blue-100">
        <h2 className="text-2xl font-bold mb-6 text-blue-700 text-center flex items-center justify-center gap-2">
          <span>🛫</span> Vuelos SBGR → SCEL (AeroAPI Departures)
        </h2>
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-3 mb-6 justify-center items-center">
          ...
          <input
            ...
            className="border border-blue-300 px-3 py-2 rounded-lg focus:outline-blue-400 w-44 text-center"
            required
          />
            ...
            <table className="min-w-full bg-white border border-blue-200 text-sm rounded-lg">
              <thead>
                <tr className="bg-blue-100 text-blue-800">
                  ...
                  <th className="border px-2 py-1">Salida Prog.</th>
                  <th className="border px-2 py-1">Llegada Prog.</th>
                </tr>
              </thead>
              <tbody>
                {flights.map((f, idx) => (
                  <tr key={f.ident + idx} className="hover:bg-blue-50">
                    ...
                    <td className="border px-2 py-1 text-center">{f.operator}</td>
                    <td className="border px-2 py-1 text-center">{f.aircraft_type}</td>
                    <td className="border px-2 py-1 text-center">{f.status}</td>
                    <td className="border px-2 py-1 text-center">{f.scheduled_out ? new Date(f.scheduled_out).toLocaleString() : ""}</td>
                    <td className="border px-2 py-1 text-center">{f.scheduled_in ? new Date(f.scheduled_in).toLocaleString() : ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {flights.length === 0 && !loading && !error && (
          <div className="text-center text-gray-500 mt-8">No se encontraron vuelos para esa ruta y fecha.</div>
        )}
      </div>
    </div>
  );
